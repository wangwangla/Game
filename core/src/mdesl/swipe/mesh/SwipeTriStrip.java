...
public class SwipeTriStrip {
	...
	private int generate(Array<Vector2> input, int mult) {
		...
		if (endcap<=0) {
            ...
            float []x = new float[]{endcap,endcap,endcap};
			perp.set(p).sub(p2).mul(new Matrix3());
		...
		for (int i=1; i<input.size-1; i++) {
			...
			perp.mul(new Matrix3(new float[]{thick/2f,thick/2f,thick/2f}));
			
			//decide on which side we are using
			perp.mul(new Matrix3(new float[]{mult,mult,mult}));
		...
		if (endcap<=0) {
			...
			perp.set(p2).sub(p).mul(new Matrix3(new float[]{endcap,endcap,endcap}));
