...
public class Actor implements YSortable {
	...
	public TextureRegion getSprite() {
		if (currentMode == MOVEMENT_MODE.WALKING) {
			if (state == MOVEMENT_STATE.MOVING) {
				return (TextureRegion)animations.getWalking(facing).getKeyFrame(moveTimer);
			...
			if (state == MOVEMENT_STATE.MOVING) {
				return (TextureRegion)animations.getBiking(facing).getKeyFrame(moveTimer);
			} else if (state == MOVEMENT_STATE.STILL) {
				return (TextureRegion)animations.getBiking(facing).getKeyFrames()[1];
			} else if (state == MOVEMENT_STATE.REFACING) {
				return (TextureRegion)animations.getBiking(facing).getKeyFrames()[0];
			}
		} else if (currentMode == MOVEMENT_MODE.RUNNING) {
			if (state == MOVEMENT_STATE.MOVING) {
				return (TextureRegion)animations.getRunning(facing).getKeyFrame(moveTimer);
			} else if (state == MOVEMENT_STATE.STILL) {
				return animations.getStanding(facing);
			} else if (state == MOVEMENT_STATE.REFACING) {
				return (TextureRegion)animations.getRunning(facing).getKeyFrames()[0];
			}
		}
